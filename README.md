...
배열에 숫자들을 담아 특정 값이 포함되어 있을 경우 `count`를 증가시키면 되겠습니다.
...
* 배열을 탐색하며 3, 6, 9가 포함된 숫자마다 count를 증가시킨다.
...
* [X] 입력받은 숫자를 쪼개서 배열에 저장한다.
	...
	- `반복문`을 이용해 number까지의 숫자를 배열에 `append` 한다.

* [X] 배열을 탐색하며 3, 6, 9가 포함된 숫자마다 count를 증가시킨다.

	- `filter()` 메소드를 이용해 조건에 맞을 경우 `count`를 1 증가시킨다.