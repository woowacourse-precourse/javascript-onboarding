...
- [ ] 문제 6 구현
  - [ ] feat: 이메일 도메인이 `email.com`인지 판별함수를 만들어 도메인이 다르다면 이메일을 제외시키기
  - [ ] feat: 닉네임이 한글이 아니라면 해당 이메일 제외시키기
  - [ ] feat: 닉네임을 연속된 두 글자로 나눈 후, `Map 객체`로 개수 관리하기(키: 연속된 두 글자, 값: 개수)
  - [ ] feat: `Map 객체`의 값이 2이상인 키를 요소로 가지는 배열 만들기
  - [ ] feat: 중복된 닉네임을 신청한 이메일만 거르기
  - [ ] feat: 이메일을 오름차순으로 정렬하고 중복 제거하기
  - [ ] feat: 구현 완료하기
- [ ] 문제 6 리팩토링
