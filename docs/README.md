...
- 문제 2 구현
  ...
  <!-- 새로운 풀이로 구현 -->
  - [x] feat: 연속하여 중복되는 문자열을 제거하는 함수 민들기
  - [x] feat: `while` 반복문을 통해 중복되는 문자열이 없을 때 까지 반복하여 구현 완료하기
- 문제 2 리팩토링
...
- 문제 7 구현
  ...
  - [x] feat: 친구 추천 점수를 관리하는 `Map 객체`를 만들고 점수 저장하기(키: user, 값: 점수)
...
- 문제 7 리팩토링
  ...
  - [x] refactor: `flatMap`메서드 `map`메서드로 변경
- 문제 7 버그 수정
