...
>
> #### 5. 사용제한될 이메일을 return 해주는 함수 => _refuseEmail 함수_
>
> - 파라미터 값으로 입력 모든 유저의 이메일 정보와 닉네임을 전달받는다. (parameter : accountInfo)
...
>
> #### 3. 함께아는 친구의 수에 대한 정보를 점수화 시켜 카운트해준다. => \_ _cntKnowScore 함수_
>
...
>
> #### 5. 점수 정보 중에서 이미 친구인 사람들을 제외해주고 sortList를 통해 정렬한다. 이후, 결과값을 생성한다. => _makeResult 함수_
>
...
> - 이때, 2번 인덱스부터 for loop 를 도는 이유는 0번 인덱스는 1번 부터의 인덱스 정보를 맞추기위해 넣어주는 garbage 값이고, 1번 인덱스는 User의 정보가 담겨있기때문에 순회할 필요가 없기 때문이다.
