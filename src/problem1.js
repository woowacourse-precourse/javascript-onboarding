...
function findMaxValue(arg) {
  ...
  let maxSumLeft = Math.max(sumValue(leftPage), multiplyValue(leftPage));
  let maxSumRight = Math.max(sumValue(rightPage), multiplyValue(rightPage));
...
function problem1(pobi, crong) {
  ...
  if (findMaxValue(pobi) > findMaxValue(crong)) return 1;
  if (findMaxValue(pobi) === findMaxValue(crong)) return 0;
  if (findMaxValue(pobi) < findMaxValue(crong)) return 2;
  return -1;
}
