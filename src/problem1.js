function problem1(pobi, crong) {
  ...
  const pobinums = []; // 포비에 받은 숫자 하나씩 분해
  const crongnums = []; // 크롱에 받은 숫자 하나씩 분해
  for (i = 0; i < N; i++) {
    const _pobinums = [];
  ...
  const pobi_max = Math.max(Math.max(...pobi_plus), Math.max(...pobi_multiply));
  const crong_max = Math.max(Math.max(...crong_plus), Math.max(...crong_multiply));
