function makeSlicedNumber(page) {
  return (page).toString().split('').map(num => parseInt(num))
}
...
function problem1(pobi, crong) {
  ...
  const maxLeftNumberPobi = Math.max(sumOfSlicedLeftNumberPobi, productOfSlicedLeftNumberPobi);
  const maxRightNumberPobi = Math.max(sumOfSlicedRightNumberPobi, productOfSlicedRightNumberPobi);
  ...
  const maxLeftNumberCrong = Math.max(sumOfSlicedLeftNumberCrong, productOfSlicedLeftNumberCrong);
  const maxRightNumberCrong = Math.max(sumOfSlicedRightNumberCrong, productOfSlicedRightNumberCrong);
