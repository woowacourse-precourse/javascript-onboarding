...
const init = (pobi, crong) => {
  const bestValueOfPobi = Math.max(...pobi.map((page) => findMaxOfPageGame(page))); // 포비의 베스트 숫자 찾기
  const bestValueOfCrong = Math.max(...crong.map((page) => findMaxOfPageGame(page))); // 크롱의 베스트 숫자 찾기
...
/**
 * 들어온 페이지 번호의 각 자리 숫자를 모두 더하거나, 모두 곱해 가장 큰 수를 구한다.
 * @param {number} page 계산하고 싶은 한 쪽 페이지 번호
 * @returns {number}
 */
const findMaxOfPageGame = (page) => {
  // 받은 페이지의 각 자리 숫자를 리스트로 변환해주는 작업
  const pageNumberList = page.toString().split('').map((num) => parseInt(num));
  // 각 자리 숫자를 모두 더했을 때
