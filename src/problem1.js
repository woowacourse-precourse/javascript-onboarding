function problem1(pobi, crong) {
  ...
  const sumPagesByPobi = pobi.map((page) => {
  ...
  const sumPagesByCrong = crong.map((page) => {
    const split = String(page).split("").map(Number);
    const sum = split.reduce((acc, cur) => acc + cur);
  ...
  const multiplyPagesByPobi = pobi.map((page) => {
    const split = String(page).split("").map(Number);
  ...
  const multiplyPagesByCrong = crong.map((page) => {
    const split = String(page).split("").map(Number);
    const multiply = split.reduce((acc, cur) => acc * cur);
