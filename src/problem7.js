function problem7(user, friends, visitors) {
  var answer = []
  var myfriendSet = new Set();
  var notMyFriend = new Set();
  var allFriend = new Set();
  var pointDic = {}

  friends.forEach(friend=> {
    if (friend[0] == user) {
      myfriendSet.add(friend[1])
    } else if (friend[1] == user) {
      myfriendSet.add(friend[0])
    }
    allFriend.add(friend[0]).add(friend[1])
  })
  allFriend.delete(user)
  var notMyFriend = new Set([...allFriend].filter(x => !myfriendSet.has(x)))
  // console.log(notMyFriend)

  friends.forEach(friend=> {
    if (myfriendSet.has(friend[0])) {
      if (notMyFriend.has(friend[1])) {
        if (pointDic[friend[1]] === undefined) {
          pointDic[friend[1]] = 10
        }else {
          pointDic[friend[1]] += 10
        }

      }
    } else if (notMyFriend.has(friend[0])) {

      if (myfriendSet.has(friend[1])) {
        if (pointDic[friend[1]] === undefined) {
          pointDic[friend[1]] = 10
        }else {
          pointDic[friend[1]] += 10
  ...
  visitors.forEach(friend=> {
    if (myfriendSet.has(friend)) {
      return
    }
    if (pointDic[friend] === undefined) {
      pointDic[friend] = 1
    }else {
      pointDic[friend] += 1
    }
  ...
  var sortable = Object.entries(pointDic)
    .sort(function(v1,v2){
      if (v1[1] > v2[1]){
          return -1;
      }
  ...
  for (let i = 0; i < Math.min(sortable.length, 5); i++){
