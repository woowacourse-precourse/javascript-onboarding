function problem7(user, friends, visitors) {
  let recommendedArr = [];
  // user 친구 리스트 추출
  let friendsOfUser = friends.filter(
    (element) => element[0] === user || element[1] === user
  );
  friendsOfUser = friendsOfUser.reduce(function (acc, cur) {
    return [...new Set(acc.concat(cur))].filter(
      (element) => !element.includes(user)
    );
  });
  ...
  // 방문자 중복제거 리스트 추출
  let visitorsOfUser = [...new Set(visitors)].filter(
    (x) => !friendsOfUser.includes(x)
  );
...
function problem7(user, friends, visitors) {
  let answer = [];
  let [recommendedArr, friendsOfUser] = getRecommendFriend(
    user,
    friends,
    visitors
  );
